---
sidebar_label: Chrome ZIP ইনস্টলেশন গাইড
title: Chrome এ AI one ZIP ফাইল কীভাবে ইনস্টল করবেন
description: Chrome এ AI one এক্সটেনশনটি ZIP ফাইল ব্যবহার করে ম্যানুয়ালি ইনস্টল করার গাইড। ডেভেলপার মোড সক্ষম করুন এবং লোড করুন।
---

# Chrome ZIP এক্সটেনশন স্থানীয় ইনস্টলেশন গাইড

## প্লাগইন ডাউনলোড করুন

Chat instructions প্লাগইন জিপ ইনস্টলেশন প্যাকেজ (ChatGPT_Shortcut-zip-3.x.x.zip) ডাউনলোড করুন এবং এটি আনজিপ করুন। (আনজিপ করা ফোল্ডারটি কম্পিউটারে রাখতে হবে, তাই এটি একটি স্থায়ী স্থানে আনজিপ করতে ভুলবেন না)

- **GitHub**: [GitHub Release](https://github.com/rockbenben/Chat-instructions/releases/latest)
- **ঘরোয়া ডাউনলোড**: [Alist Cloud](https://alist.newzone.top:9003/apps/ChatGPT%20Shortcut%20Extension); [Lanzou Cloud](https://wwva.lanzouq.com/b01lsc9vi), পাসওয়ার্ড: 1qow

## ডেভেলপার মোড সক্ষম করুন

ক্রোমের "এক্সটেনশন পরিচালনা করুন" পৃষ্ঠা খুলুন, এবং "ডেভেলপার মোড" চালু করুন।

নিচের ঠিকানাটি কপি করুন এবং ব্রাউজার অ্যাড্রেস বারে পেস্ট করুন, খুলতে এন্টার টিপুন। পৃষ্ঠার উপরের ডান কোণে "ডেভেলপার মোড" চালু করুন।

```txt
chrome://extensions
```

![](https://img.newzone.top/2024-08-12-22-05-52.png?imageMogr2/format/webp)

## প্লাগইন ইনস্টল করুন

ইনস্টলেশন সম্পূর্ণ করতে আনজিপ করা ফোল্ডারটি এক্সটেনশন পৃষ্ঠায় টেনে আনুন।

![](https://img.newzone.top/2024-08-12-22-27-47.png?imageMogr2/format/webp)

সফল ইনস্টলেশনের পরে, আপনি [এক্সটেনশন ব্যবহার টিউটোরিয়াল](./usage.md) দেখতে পারেন।
